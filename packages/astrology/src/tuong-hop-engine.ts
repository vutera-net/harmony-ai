...
import { calculateBatTrach } from './bat-trach-engine'
import type { TuongHopResult, CungMenh } from './types'
...
export function calculateTuongHop(
  ...
  // Lấy Can Chi năm
  const can1 = getCanChiYear(year1).can;
  const can2 = getCanChiYear(year2).can;
  const chi1 = getCanChiYear(year1).chi;
  const chi2 = getCanChiYear(year2).chi;
  ...
  const cung1 = calculateBatTrach(year1, gender1).cungMenh;
  const cung2 = calculateBatTrach(year2, gender2).cungMenh;
  ...
  const cungMatrix: Record<CungMenh, Record<CungMenh, { text: string; score: number }>> = {
