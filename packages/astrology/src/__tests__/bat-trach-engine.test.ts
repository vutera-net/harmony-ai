...
describe('Bát Trạch Engine - getMenhNhom', () => {
  it('should classify Khảm as dong', () => {
    expect(getMenhNhom('Khảm')).toBe('dong')
  })

  it('should classify Chấn as dong', () => {
    expect(getMenhNhom('Chấn')).toBe('dong')
  })

  it('should classify Tốn as dong', () => {
    expect(getMenhNhom('Tốn')).toBe('dong')
  })
  ...
  it('should classify Khôn as tay', () => {
    expect(getMenhNhom('Khôn')).toBe('tay')
  })

  it('should classify Càn as tay', () => {
    expect(getMenhNhom('Càn')).toBe('tay')
  })

  it('should classify Đoài as tay', () => {
    expect(getMenhNhom('Đoài')).toBe('tay')
  })

  it('should classify Cấn as tay', () => {
    expect(getMenhNhom('Cấn')).toBe('tay')
  })
})

describe('Bát Trạch Engine - getBatTrachHuongs', () => {
  it('should return exactly 8 directions for Khảm', () => {
    const huongs = getBatTrachHuongs('Khảm')
    expect(huongs).toHaveLength(8)
  })

  it('should have 4 good (isTot=true) and 4 bad (isTot=false) directions', () => {
    const cungList = ['Khảm', 'Khôn', 'Chấn', 'Tốn', 'Càn', 'Đoài', 'Cấn', 'Ly'] as const
  ...
  it('Khảm - Sinh Khi should be DN (Đông Nam)', () => {
    const huongs = getBatTrachHuongs('Khảm')
...
describe('Bát Trạch Engine - calculateBatTrach', () => {
  ...
  it('1984 male → Đoài, tay', () => {
    const result = calculateBatTrach(1984, 'male')
    expect(result.cungMenh).toBe('Đoài')
    expect(result.cungNumber).toBe(7)
    expect(result.nhomMenh).toBe('tay')
  })

  it('1984 female → Cấn, tay', () => {
  ...
  it('1990 male → Khảm, dong', () => {
    const result = calculateBatTrach(1990, 'male')
    expect(result.cungMenh).toBe('Khảm')
...
describe('Bát Trạch Engine - getCungMenhInfo', () => {
  it('should return description for Khảm', () => {
    const info = getCungMenhInfo('Khảm')
    expect(typeof info).toBe('string')
    expect(info.length).toBeGreaterThan(0)
  })

  it('should return description for all cung', () => {
    const cungs = ['Khảm', 'Khôn', 'Chấn', 'Tốn', 'Càn', 'Đoài', 'Cấn', 'Ly'] as const
