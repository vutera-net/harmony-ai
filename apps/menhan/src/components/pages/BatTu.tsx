...
interface BatTuResult {
  year: { can: string; chi: string; canHanh: string; chiHanh: string };
  month: { can: string; chi: string; canHanh: string; chiHanh: string };
  day: { can: string; chi: string; canHanh: string; chiHanh: string };
  hour: { can: string; chi: string; canHanh: string; chiHanh: string };
  nguHanhCount: RadarData;
  khuyet: string[];
  vuong: string[];
}
...
export default function BatTu() {
    ...
    async function fetchProfile() {
      try {
        ...
        if (res.ok) {
          ...
          setProfile(data);
          if (data.birthYear) {
            setDateStr(`${data.birthYear}-01-01`);
          }
        }
      } catch (e) {
  ...
  const handleCalculate = async () => {
    ...
    try {
      const res = await fetch("/api/battu/analyze", {
        method: "POST",
