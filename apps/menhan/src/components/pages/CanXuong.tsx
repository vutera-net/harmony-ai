...
import { Scale, Info, Sparkles, ChevronDown, RefreshCw } from "lucide-react";
import { THIEN_CAN, DIA_CHI } from "@harmony/astrology";
...
export default function CanXuong() {
  const [profile, setProfile] = useState<{ birthYear?: number; fullName?: string } | null>(null);
  ...
  const handleCalculate = async () => {
    if (typeof year === "number" && year >= 1900 && year <= 2030) {
      ...
      try {
        const res = await fetch("/api/can-xuong/analyze", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ year, month, day: safeDay, hourIdx }),
        });
        if (res.ok) {
          const data = await res.json();
          setResult(data);
  ...
  const getYearCanChi = (y: number) => {
    const canIndex = (y - 4) % 10;
    const chiIndex = (y - 4) % 12;
    return `${THIEN_CAN[(canIndex + 10) % 10]} ${DIA_CHI[(chiIndex + 12) % 12]}`;
  };
        ...
        <div className="bg-white/80 dark:bg-stone-800/80 backdrop-blur-md p-8 rounded-[2rem] border-2 border-stone-100 dark:border-stone-700 shadow-xl shadow-stone-900/5">
          ...
          {profile && (
            <div className="mb-6 p-3 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/50">
               <p className="text-xs text-amber-700 dark:text-amber-400 font-medium">
                 ✦ Đã điền từ hồ sơ: {profile.fullName} · {profile.birthYear ? getYearCanChi(profile.birthYear) : ""}
               </p>
            </div>
          )}
          ...
          <div className="space-y-5">
            ...
            <button
              ...
              disabled={year === "" || year < 1900 || year > 2030 || isCalculating}
