...
export default function TuongHop() {
    ...
    async function fetchProfile() {
      try {
        ...
        if (res.ok) {
          ...
          setProfile(data);
          if (data.birthYear) {
            setYear1(data.birthYear);
            setGender1(data.gender === "MALE" ? "male" : "female");
          }
        }
      } catch (e) {
  ...
  const handleCalculate = async () => {
    if (typeof year1 === "number" && typeof year2 === "number") {
      ...
      try {
        const res = await fetch("/api/tuong-hop/analyze", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ year1, gender1, year2, gender2 }),
        });
        if (res.ok) {
          const data = await res.json();
          setResult(data);
    ...
    <div className="max-w-4xl mx-auto py-6 px-4">
      ...
      <div className="bg-white/90 dark:bg-stone-800/80 backdrop-blur-md p-6 md:p-8 rounded-[2rem] border-2 border-stone-100 dark:border-stone-700 shadow-xl shadow-stone-900/5 mb-10">
        ...
        <button
          ...
          disabled={year1 === "" || year2 === "" || year1 < 1900 || year1 > currentYear || year2 < 1900 || year2 > currentYear || isCalculating}
